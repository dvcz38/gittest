...
//在相对路径中发布端点websocket
@ServerEndpoint("/deviceinfows.do")
public class DeviceInfoWS {
	
//  MyThread thread1=new MyThread();
	DeviceInfoWSThread thread =null;
//  Thread thread=new Thread(thread1);
  
  //用来存放每个客户端对应的MyWebSocket对象。
  private static CopyOnWriteArraySet<DeviceInfoWS> webSocketSet = new CopyOnWriteArraySet<DeviceInfoWS>();
  ...
  public void onOpen(Session session) throws IOException{
      ...
      //开启一个线程对数据库中的数据进行轮询 
      thread=new DeviceInfoWSThread(session); 
      thread.start();
//      thread.start();
...
/**
  ...
  public void sendMessage() throws IOException{
      //群发消息
      for(DeviceInfoWS item: webSocketSet){
